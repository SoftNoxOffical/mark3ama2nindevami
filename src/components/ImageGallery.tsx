...
const ImageGallery: React.FC<ImageGalleryProps> = ({ images, productName }) => {
  ...
  const openModal = (index: number) => {
    // 1) önce sayfayı galerinin ana görseline kaydır
    mainImageRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });

    // 2) ardından modalı aç (scroll’un başlaması için ufak bir gecikme iyi olur)
    requestAnimationFrame(() => {
      setModalIndex(index);
      setIsModalOpen(true);
    });
    // Alternatif: setTimeout(() => { setModalIndex(index); setIsModalOpen(true); }, 200);
  };
  const closeModal = () => {
  ...
  return (
    <>
      {/* Main Gallery */}
      <div className="space-y-4" ref={mainImageRef}>
        {/* Main Image */}
          ...
          <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-2">
            {images.map((image, index) => (
              <button
                ...
                className={`flex-shrink-0 w-16 h-16 sm:w-20 sm:h-20 rounded-lg overflow-hidden border-2 transition-all duration-300 hover:scale-105 ${index === currentIndex
            ...
            <div className="absolute bottom-6 sm:bottom-10 left-1/2 -translate-x-1/2 flex gap-2 max-w-full overflow-x-auto px-4">
              {images.map((image, index) => (
                <button
                  ...
                  className={`flex-shrink-0 w-12 h-12 sm:w-16 sm:h-16 rounded-lg overflow-hidden border-2 transition-all ${index === modalIndex
